...
诊所预约与健康随访移动端，覆盖预约挂号、签到、候诊、接诊状态、健康档案和复诊任务。演示数据均为虚构，不涉及诊断、处方、支付或真实患者隐私。
...
开发服务器默认把 `/api/*` 代理到 `http://localhost:8080`，与同目录的 `careflow-admin/server` 默认端口一致。也可以通过环境变量修改：
...
页面默认请求 `/api/v1`，生产环境可通过 `VITE_API_BASE_URL` 指向独立的 CareFlow API 服务。所有写操作会自动生成 `Idempotency-Key`，避免重复预约、重复签到和重复完成随访。

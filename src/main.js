...
const demoAppointments = [
  { id: 'AP-0716-082', patientId: 'PT-001', patient: '许汝林', department: '全科门诊', doctor: '林医生', scheduledAt: '今天 09:30', status: '待签到' },
  { id: 'AP-0716-079', patientId: 'PT-001', patient: '许汝林', department: '皮肤科', doctor: '沈医生', scheduledAt: '今天 14:00', status: '候诊中' },
  { id: 'AP-0715-031', patientId: 'PT-001', patient: '许汝林', department: '康复理疗', doctor: '赵医生', scheduledAt: '07/23 10:30', status: '已完成' },
]
...
function renderAppointment(appointment) {
  ...
  return `<article class="visit">
    ...
    <p>${escapeHtml(appointment.doctor)} · 上海静安联合诊所</p>
    ${actionButton}
  </article>`
}
...
function render() {
  app.innerHTML = `<main class="app">
    <header>
      <div><p>CAREFLOW / 2026</p><h1>把健康交给<br><b>值得信赖的人</b></h1></div>
      <div class="header-side"><span class="source-badge">${dataSource}</span><span class="avatar">许</span></div>
    </header>
    <section class="hero"><span>预约工作台</span><h2>今天也要好好照顾自己</h2><p>预约 · 候诊 · 随访<br>每一步都有清晰提醒</p><div class="sun">✚</div></section>
    <section class="quick">
      <button data-action="create-appointment"><b>＋</b><span>预约挂号</span></button>
      <button data-action="refresh"><b>◷</b><span>刷新候诊</span></button>
      <button data-action="create-followup"><b>♡</b><span>新建随访</span></button>
    </section>
    ...
    <div class="section-head"><h3>随访任务 <small class="coral">${followups.filter((item) => item.status !== '已完成').length} 条待办</small></h3><a data-action="refresh">查看 →</a></div>
    <section class="reminders">${followups.length ? followups.slice(0, 3).map(renderFollowup).join('') : '<div class="empty">暂无随访任务</div>'}</section>
...
function localAppointment() {
  return {
    ...
    patientId: 'PT-001', patient: '许汝林', department: '全科门诊', doctor: '林医生',
...
async function refreshFromApi() {
  ...
  render()
  showToast(synced ? '已同步最新预约与随访' : '服务暂不可用，继续使用演示数据')
}
...
async function transitionAppointment(id, action) {
  ...
  try {
    const updated = action === 'checkin'
      ...
      : await api.updateAppointmentStatus(id, statusByAction[action], '患者端')
